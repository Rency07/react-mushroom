...
var CONNECT_STR = "mongodb://10.9.160.191:27017/react";
...
//  moreValData   {$or:[{year:"1993"},{year:"1994"}]}
   ...
   //去重后字段
  findSimpleData(db, collectionName, simpleData,callback){
//  simpleDate为字符 year
	 db.collection(collectionName).distinct(simpleData, (err, result) => {
	     if(err){
	       console.log(err)
	     }else{
	       callback(result);
   ...
   //修改   给whereobj的语句更新updateobj
  upData(db, collectionName, whereobj, updateobj, callback){
